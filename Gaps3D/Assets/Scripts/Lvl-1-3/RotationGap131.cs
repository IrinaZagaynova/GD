...
public class RotationGap131 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsClose(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(3.2f, gameObject.transform.position.y, -0.3f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(0.55f, gameObject.transform.position.y, -0.1f);
			...
			{
				transform.position = new Vector3(3.1f, 0, -2.8f);
