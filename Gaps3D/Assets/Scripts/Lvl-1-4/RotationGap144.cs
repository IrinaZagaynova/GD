...
public class RotationGap144 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsClose(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(6.75f, -2.35f, -0.2f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(4.6f, -2.35f, -0.2f);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(4.6f, gameObject.transform.position.y, -2.2f);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(6.6f, -2.3f, -2.7f);
