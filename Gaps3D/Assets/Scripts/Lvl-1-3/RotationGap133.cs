...
public class RotationGap133 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsClose(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(-7.5f, gameObject.transform.position.y, -0.35f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(-7.6f, gameObject.transform.position.y, -2.7f);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(-5, gameObject.transform.position.y, -2.9f);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(-4.9f, 0, -0.3f);
