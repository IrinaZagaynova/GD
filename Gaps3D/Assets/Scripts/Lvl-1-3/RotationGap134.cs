...
public class RotationGap134 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsClose(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(-0.9f, gameObject.transform.position.y, -3);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(-0.4f, gameObject.transform.position.y, -0.3f);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(-3.3f, gameObject.transform.position.y, 0.5f);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(-3.9f, 0, -2.7f);
