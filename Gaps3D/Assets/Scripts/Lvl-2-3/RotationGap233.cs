...
public class RotationGap233 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		{
			transform.position = new Vector3(6.4f, gameObject.transform.position.y, gameObject.transform.position.z);
			_position = Position.Back;
		}
		else if (_position == Position.Back)
		{
			transform.position = new Vector3(gameObject.transform.position.x, gameObject.transform.position.y, -2.2f);
