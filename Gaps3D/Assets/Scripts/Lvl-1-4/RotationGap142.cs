...
public class RotationGap142 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsClose(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(gameObject.transform.position.x, gameObject.transform.position.y, -3.1f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(-0.5f, gameObject.transform.position.y, -2.75f);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(-0.6f, gameObject.transform.position.y, -0.5f);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(-3.25f, -2.35f, -0.1f);
